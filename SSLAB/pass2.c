...
void main()
{
char a[10],ad[10],label[10],op
code[10],operand[10],mnemonic[10],symbol[10];
...
t%s
\
t",address,label,opcode,operand);
len=strlen(operand);
actual_len=len
-
3;
...
t%d0000
\
n",address,label,opcode,operand,
code); 
}
else
{
...
t%d%d
\
n",address,label,opcode,operand,code,add);
}
}
fscanf(fp3,"%d%s%s%s",&
address,label,opcode,operand);
